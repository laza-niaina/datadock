...
function config(engine: 'mysql' | 'mariadb' | 'sqlite'): ConnectionConfig {
  return {
    profile: {
      id: 'p1',
      name: 'p1',
...
describe('registerBuiltinDrivers', () => {
  it('registers exactly mysql, mariadb and sqlite on its own registry', () => {
  ...
  it('gives MySQL and MariaDB distinct factories with correct server defaults', () => {
    const registry = new DriverRegistry('test');
    registerBuiltinDrivers(registry);
